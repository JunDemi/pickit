...
export default function TagSearchAutoComplete({ size = 'large' }: Props) {
  ...
  const makeTag: (label: string) => void = (label) => {
    ...
    setTags((prev) =>
      Array.from(
        new Set([...prev, label.charAt(0) === '#' ? label : '#' + label]),
      ),
    );
    setInputValue('');
    setIndexDropdownItem(0);
  ...
  const onKeyDownInput: ({
    key,
  }: React.KeyboardEvent<HTMLInputElement>) => void = ({ key }) => {
    switch (key) {
      case 'Enter':
      ...
      case 'ArrowUp':
        ...
        break;
      case 'ArrowDown':
        MOCK_DATA.length > indexDropdownItem + 1 &&
          setIndexDropdownItem((index) => ++index);
        break;
      default:
        break;
    }
  ...
  return (
    <div className={`tag-search-container ${size}`}>
      <div
        className={`tag-search-content-wrapper ${
      ...
      >
        <>
          <div className="tag-search-input-wrapper">
            ...
            <input
              className="tag-search-input-autocomplete"
        ...
        <button className="tag-search-btn-search" onClick={searchInput}>
          <SearchImage />
        </button>
      </div>
      {searching && (
          ...
          <>
            ...
            {MOCK_DATA.length > 0 &&
              MOCK_DATA.map((item, index) => (
                <li
                  key={index}
                  className={`${indexDropdownItem === index ? 'selected' : ''}`}
                  value={item.value}
                >
                  {item.label}
                </li>
